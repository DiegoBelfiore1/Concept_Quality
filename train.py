...
from CUB.dataset import load_data, find_class_imbalance, print_placeholder_summary
...
def run_epoch(model, optimizer, loader, loss_meter, acc_meter, criterion, attr_criterion, args, is_training):
    ...
    for i, data in enumerate(loader):
        print(f"Processing batch {i+1}...") #modified to show the current batch 
...
def train(model, args):
    ...
    stop_epoch = min(args.epochs, int(math.log(MIN_LR / args.lr) / math.log(LR_DECAY_SIZE)) * args.scheduler_step) #Correcting the log bug
    ...
    for epoch in range(0, args.epochs):
        print(f"Started working on epoch {epoch+1}...")
        ...
        if epoch >= 100 and val_acc_meter.avg < 3: 
            ...
            break
        if epoch - best_val_epoch >= 5: #early stopping modified, original "100"
            print("Early stopping because acc hasn't improved for a long time")
            break
    print_placeholder_summary()
