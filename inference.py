...
def eval(args):
    ...
    for data_idx, data in enumerate(loader):
        ...
        if args.attribute_group:
            ...
            #attributes = torch.cat(outputs, dim=1).detach().cpu().numpy()
            attributes_only = outputs[1:]  #only takes attributes. 
            attributes = torch.cat(attributes_only, dim=1).detach().cpu().numpy()
            extracted_attributes.append(attributes) #connected to line 239
    ...
    if args.use_attr and not args.no_img:  # print some metrics for attribute prediction performance
        ...
        print(report + '\n')
    #features extraction
    all_attributes = np.concatenate(extracted_attributes, axis=0)

    #saving
    csv_output_path = os.path.join(args.log_dir, "inception_attributes.csv")
    np.savetxt(csv_output_path, all_attributes, delimiter=",")
    #printing the shape of the features
    print(f"Saved features of dim {all_attributes.shape} in {csv_output_path}")
...
if __name__ == '__main__':
    ...
    print(f"Log directory: {log_dir}")  #debug
    if not os.path.exists(log_dir): #creating the directory
        os.makedirs(log_dir)  
