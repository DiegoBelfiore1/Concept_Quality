...
def get_few_shot_data(n_samples, out_dir, data_file='train.pkl'):
    ...
    for c, data_list in class_to_data.items():
        ...
        #else:
            new_data.extend(random.sample(data_list,  int(len(data_list) * 0.3))) #to allow a 30% dataset
...
def create_logits_data(model_path, out_dir, data_dir='', use_relu=False, use_sigmoid=False):
    ...
    """
    model = torch.load(model_path, weights_only=False) #fixed pytorch issue
...
if __name__ == '__main__':
    ...
    if args.exp == 'ExtractConcepts':
        ...
        #get_few_shot_data(args.n_samples, args.out_dir, os.path.join(args.splits_dir, 'test.pkl'))
