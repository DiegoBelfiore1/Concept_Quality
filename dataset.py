...
placeholder_counter = {"train": 0, "val": 0, "test": 0}
...
class CUBDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        try:
          ...
          # if there is no picture model goes on and generates a placehoder
            print(f"File not found: {img_path}. Generating placeholder...")
            if "val" in img_path:
              split = 'val'
            elif self.is_train:
              split = 'train'
              ...
              split = 'test'
            placeholder_counter[split] += 1  
            img = Image.new('RGB', (224, 224), color='white')  # Placeholder image
          print(f"Placeholder count: {placeholder_counter}")
...
def print_placeholder_summary():
    """Placeholders recap."""
    print("\n--- Placeholder Summary ---")
    print(f"Placeholders in train dataset: {placeholder_counter['train']}")
    print(f"Placeholders in val dataset: {placeholder_counter['val']}")
    print(f"Placeholders in test dataset: {placeholder_counter['test']}")
    print("---------------------------\n")
...
def load_data(pkl_paths, use_attr, no_img, batch_size, uncertain_label=False, n_class_attr=2, image_dir='images', resampling=False, resol=299):
    ...
    if resampling:
        ...
        loader = DataLoader(dataset, batch_size=batch_size, shuffle=shuffle, drop_last=drop_last, num_workers=8, pin_memory=True) #CPU handling
    return loader
